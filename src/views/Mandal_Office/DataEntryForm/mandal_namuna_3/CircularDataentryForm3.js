...
const CircularDataentryForm3 = ({ isEdit = null }) => {
  ...
  const navigateToDataentryPage = () => {
    navigate('/Mandal_Office/DataEntryForm/CircularVillageForm3')
    handlerReset()
  }
  const handlerReset = () => {
    setAllFormVal({
  ...
  const handleSave = (e) => {
    e.preventDefault()
    console.log('Mandal Namuna 3 Form Values:->', allFormVal)
    const inputParams = {
    ...
    axios
      .post(`${URLS?.MandalURL}/mandalNamuna3/saveMandalNamuna3Data`, inputParams, {
        headers: reqHeaders,
      })
      .then((val) => {
            ...
            <CCardHeader>
              <CRow>
                <center>
                  <CFormLabel>
                    <h5>
                      <strong> मंडळ नमुना तीन (Dataentry)</strong>
                    </h5>
                  </CFormLabel>
                </center>
              </CRow>
...
export default CircularDataentryForm3
