...
const VillageForm1LiveTable = () => {
  ...
  const showsatbara = async (surveyNo) => {
    ...
    await axios
      ...
      .then((res) => {
        ...
        var encryptedData =
          ''
        const key = 'FCk6WvUiq70RKTpnwf0ASuMWl2ki6rXdhCFaaYFOfOhIXNoeC3dL6YnHWwRPS1Jy'
        const iv = 'C3dL6YnHWwRPS1Jy'

        var data = decryptData(encryptedData, key, iv)

        // console.log(encryptedData)
        console.log(data, '---------iv----------')

        // Decode the base64 encoded encrypted data
        const encryptedDataDecoded = CryptoJS.enc.Base64.parse(encryptedData)

        // Create a WordArray from the key and iv
        ...
        const decryptedData = CryptoJS.AES.decrypt(
          {
            ciphertext: encryptedDataDecoded,
          },
          keyWordArray,
          {
            iv: ivWordArray,
        ...
        const decryptedText = decryptedData.toString(CryptoJS.enc.Hex)

        console.log(atob(decryptedText), '-------')

        // console.log('Decrypted data (Utf8): ' + JSON.parse(data))

        // if (res.status == 200) {
        //   //console.log(res.data.data, '------------------')
        //   // var data = decryptAES256CBC(res.data.data, a, VI)
        //   // console.log(data, '--------Data fetch--------')
        //   var d = decrypt(encryptedData, key, iv)
        //   console.log(d, '--------Data fetch----0000000000000----')
        //   decryptAES256CBC(encryptedData, key, iv)
        //     .then((decryptedData) => {
  ...
  const decryptData = (encryptedData, key, iv) => {
    try {
      ...
      // Convert the decrypted data to a UTF-8 string
      const decryptedText = decryptedBytes.toString(CryptoJS.enc.Utf16)

      // Set the decrypted data state
